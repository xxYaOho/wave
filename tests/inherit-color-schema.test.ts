		...
		test('should reject inheritColor on shadow token', () => {
			...
			expect(result.issues).toContainEqual(
				expect.objectContaining({
					level: 'error',
					message:
						'inheritColor can only be used with $type "color", got "shadow"',
				}),
			);
		});
		...
		test('should reject inheritColor on border token', () => {
			...
			expect(result.issues).toContainEqual(
				expect.objectContaining({
					level: 'error',
					message:
						'inheritColor can only be used with $type "color", got "border"',
				}),
			);
		});
		...
		test('should reject inheritColor with invalid opacity type', () => {
			...
			expect(result.issues).toContainEqual(
				expect.objectContaining({
					...
					message:
						'inheritColor.property.opacity must be a number, alias (string), or $ref object',
				}),
			);
		});
		...
		test('should reject invalid inheritColor type (string)', () => {
			...
			expect(result.issues).toContainEqual(
				expect.objectContaining({
					...
					message:
						'inheritColor must be a boolean (true) or an object with optional opacity and siblingSlot',
				}),
			);
		});
	});
