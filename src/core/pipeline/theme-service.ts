...
export type ThemeGenerationResult = ThemeGenerationSuccess | ThemeGenerationFailure;

// CQ-007: Theme generation orchestration service
// Extracts CLI command logic into a reusable core service
export async function generateTheme(input: ThemeGenerationInput): Promise<ThemeGenerationResult> {
  const { themeName, themePath, cliOutput, cliPlatform, generateOptions } = input;
  ...
  const platforms = cliPlatform
    ? cliPlatform.split(',').map((s) => s.trim()).filter(Boolean)
