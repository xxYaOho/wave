...
describe('theme command current behavior', () => {
  test('main, night and variant are parsed as independent documents', async () => {
    const fixtureDir = path.join(rootDir, 'tests/fixtures/baseline-independent');
    ...
    const { exitCode } = await runWaveTheme(['-f', path.join(fixtureDir, 'themefile'), '-o', outputDir]);

    expect(exitCode).toBe(0);

    const mainJson = JSON.parse(await fs.readFile(path.join(outputDir, 'baseline-independent.json'), 'utf-8'));
    const nightJson = JSON.parse(await fs.readFile(path.join(outputDir, 'baseline-independent-night.json'), 'utf-8'));
    const darkJson = JSON.parse(await fs.readFile(path.join(outputDir, 'baseline-independent-dark.json'), 'utf-8'));
  ...
  test('palette and dimension are shared as reference sources across main, night and variant', async () => {
    const fixtureDir = path.join(rootDir, 'tests/fixtures/baseline-independent');
    const outputDir = path.join(rootDir, '.temp-test-baseline-independent-shared');
