...
describe('smoothShadow transformation', () => {
  test('derives smooth shadow layers from a single layer', () => {
    const input: ResolvedTokenGroup = {
      shadow: {
        $type: 'shadow',
        raised: {
          $value: { color: '#000000cc', offsetX: 0, offsetY: 6, blur: 12, spread: 0 },
          $extensions: {
            smoothShadow: {
              cubicBezier: [0, 0, 1, 1],
              step: 3,
  ...
  test('orders layers from small to large', () => {
    const input: ResolvedTokenGroup = {
      shadow: {
        $type: 'shadow',
        raised: {
          $value: { color: '#000000', offsetX: 0, offsetY: 10, blur: 20, spread: 0 },
          $extensions: {
            smoothShadow: {
              cubicBezier: [0, 0, 1, 1],
              step: 5,
  ...
  test('preserves inset flag across all layers', () => {
    const input: ResolvedTokenGroup = {
      shadow: {
        $type: 'shadow',
        raised: {
          $value: { color: '#000000', offsetX: 0, offsetY: 2, blur: 4, spread: 0, inset: true },
          $extensions: {
            smoothShadow: {
              cubicBezier: [0, 0, 1, 1],
              step: 3,
  ...
  test('handles rem units with 3 decimal places', () => {
    const input: ResolvedTokenGroup = {
      shadow: {
        $type: 'shadow',
        raised: {
          $value: { color: '#000000', offsetX: '0.5rem', offsetY: '1rem', blur: '2rem', spread: 0 },
          $extensions: {
            smoothShadow: {
              cubicBezier: [0, 0, 1, 1],
              step: 3,
  ...
  test('throws for invalid cubicBezier', () => {
    ...
    expect(() => transformToSDFormat(input)).toThrow('smoothShadow.cubicBezier must be an array of 4 numbers');
  });
  ...
  test('throws for step < 1', () => {
    ...
    expect(() => transformToSDFormat(input)).toThrow('smoothShadow.step must be an integer >= 1');
  });
});
