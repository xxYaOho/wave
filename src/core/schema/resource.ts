...
export function validateGenericResource(
	...
	| { success: true; namespace: string; data: Record<string, unknown> }
	...
	if (keys.length === 0) {
		return {
			...
			error: {
				line: 1,
				message: 'Resource file root must be a non-empty object',
			},
		};
	}
 ...
 * 校验 custom 资源扩展名是否合法
 */
export function validateCustomResourceExtension(
	filePath: string,
): ParseError | null {
