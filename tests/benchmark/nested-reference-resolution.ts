...
import type { ReferenceDataSources, DtcgValue, DtcgToken, DtcgTokenGroup } from '../../src/types/index.ts';
...
function benchmarkDepths(sources: ReferenceDataSources, iterations = 1000): void {
  ...
  const ref5Layer = '{leonardo.global.color.level1_5.level2_5.level3_5.level4_5.level5_5}';
...
function benchmarkMixedReferences(sources: ReferenceDataSources, iterations = 1000): void {
  console.log('\n\n🔄 混合引用基准测试');
  console.log('=' .repeat(60));
...
function benchmarkPerformanceComparison(sources: ReferenceDataSources): void {
  ...
  const ref5 = '{leonardo.global.color.level1_5.level2_5.level3_5.level4_5.level5_5}';
  ...
  console.log(`3层深度:  ${(end3 - start3).toFixed(3)}ms (平均: ${((end3 - start3) / iterations).toFixed(6)}ms/次)`);
  console.log(`5层深度:  ${(end5 - start5).toFixed(3)}ms (平均: ${((end5 - start5) / iterations).toFixed(6)}ms/次)`);
  console.log(`10层深度: ${(end10 - start10).toFixed(3)}ms (平均: ${((end10 - start10) / iterations).toFixed(6)}ms/次)`);
  
  console.log('\n📊 性能比率分析:');
  const baseTime = (end3 - start3) / iterations;
  console.log(`5层深度 vs 3层深度:  ${(((end5 - start5) / iterations) / baseTime).toFixed(2)}x`);
  console.log(`10层深度 vs 3层深度: ${(((end10 - start10) / iterations) / baseTime).toFixed(2)}x`);
}
...
function main(): void {
  ...
  console.log('\n🔧 验证测试数据...');
  const testRef3 = resolveReference('{leonardo.global.color.level1.level2.level3}', sources);
  const testRef5 = resolveReference('{leonardo.global.color.level1_5.level2_5.level3_5.level4_5.level5_5}', sources);
  const testRef10 = resolveReference('{leonardo.global.color.l1.l2.l3.l4.l5.l6.l7.l8.l9.l10}', sources);
  ...
  if (testRef5 === undefined) {
    console.warn('⚠️ 5层深度引用解析失败，可能存在数据结构问题，但继续执行其他测试');
  }
