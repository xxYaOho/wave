...
function getInheritColorSiblingSlot(
	token: TransformedToken,
): string | undefined {
	return (token as Record<string, unknown>).inheritColorSiblingSlot as
		| string
		| undefined;
}
...
function getInheritColorOpacity(token: TransformedToken): number | undefined {
	return (token as Record<string, unknown>).inheritColorOpacity as
		| number
		| undefined;
}
...
function getInheritColorAlpha(token: TransformedToken): number | undefined {
	return (token as Record<string, unknown>).inheritColorAlpha as
		| number
		| undefined;
}
...
function resolveSketchColor(
	...
	if (isInheritColorToken(token)) {
		...
		if (siblingSlot) {
			...
			if (siblingToken) {
				colorValue = extractColorFromValue(
					siblingToken.$value ?? siblingToken.value,
				);
				swatchName = (siblingToken as Record<string, unknown>)._swatchName as
					| string
					| undefined;
			}
		}
