...
describe('theme command current fallback behavior', () => {
  test('fails fast when main.yaml has unresolvable references', async () => {
    ...
    const { exitCode, stderr } = await runWaveTheme(['-f', path.join(fixtureDir, 'themefile'), '-o', outputDir]);
    ...
    const fileExists = await Bun.file(path.join(outputDir, 'baseline-fallback.json')).exists();
