...
import type { BuiltinPalette, BuiltinDimension } from '../../types/index.ts';

export type UserPalette = BuiltinPalette;
export type UserDimension = BuiltinDimension;

export async function loadUserPalette(absolutePath: string): Promise<UserPalette | null> {
  try {
...
export async function loadUserDimension(absolutePath: string): Promise<UserDimension | null> {
  try {
    const file = Bun.file(absolutePath);
