...
function transformBorderToSketchFormat(
	waveBorder: WaveBorderValue,
): SketchBorder {
	const width =
		typeof waveBorder.width === 'number'
			? waveBorder.width
...
function buildFullSketchOutput(
	...
	for (const token of tokens) {
		...
		// border 类型
			...
			const key = generateFlatKey(effectivePath);
			result[key] = transformBorderToSketchFormat(
				tokenValue as WaveBorderValue,
			);
		}
		// opacity 类型
