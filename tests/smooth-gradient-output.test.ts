...
describe('smoothGradient CLI output', () => {
	test('generates derived gradient in JSON without $extensions', async () => {
		...
		const json = JSON.parse(
			await fs.readFile(path.join(outputDir, 'smooth-gradient.json'), 'utf-8'),
		);
		const stops = json['theme-gradient-hero'] as {
			color: string;
			position: number;
		}[];
		...
		const raw = await fs.readFile(
			path.join(outputDir, 'smooth-gradient.json'),
			'utf-8',
		);
	...
	test('generates CSS gradient tokens without $extensions', async () => {
		const fixtureDir = path.join(rootDir, 'tests/fixtures/smooth-gradient');
		...
		const css = await fs.readFile(
			path.join(outputDir, 'smooth-gradient.css'),
			'utf-8',
		);
		expect(css).toContain('--theme-gradient-hero: linear-gradient(to right');
